...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		// ★ 编辑区域左右平移：Shift + A/D 或 Shift + ←/→（多 K 谱面超出屏幕时横向查看）
		if (FlxG.keys.pressed.SHIFT)
		{
			var panStep:Float = GRID_SIZE * (FlxG.keys.pressed.ALT ? 4 : 1);
		...
		// ★ 相机目标 = 网格中心：让整张网格在屏幕上水平居中（原来用 CAM_OFFSET=360 固定偏移会偏左）
		camPos.x = strumLine.x + GRID_SIZE * (totalKeys + 1) / 2 - gridPanX;
		...
		if (!blockInput)
		{
			...
			if (!blockInput)
			{
				if (FlxG.keys.justPressed.RIGHT && !FlxG.keys.pressed.CONTROL && !FlxG.keys.pressed.SHIFT)
				{
					curQuant++;
				...
				if (FlxG.keys.justPressed.LEFT && !FlxG.keys.pressed.CONTROL && !FlxG.keys.pressed.SHIFT)
				{
					curQuant--;
					if (curQuant < 0)
			...
			if ((FlxG.keys.justPressed.D && !FlxG.keys.pressed.SHIFT) || virtualPad.buttonRight.justPressed)
				changeSection(curSec + shiftThing);
			if ((FlxG.keys.justPressed.A && !FlxG.keys.pressed.SHIFT) || virtualPad.buttonLeft.justPressed)
			{
				if (curSec <= 0)
				{
	...
	/** ★ 编辑区域左右平移量（Shift+A/D 或 Shift+←/→；BACKSPACE 归零） */
	var gridPanX:Float = 0;
